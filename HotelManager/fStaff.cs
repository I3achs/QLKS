...
namespace HotelManager
{
    public partial class fStaff : Form
    {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (fCustomer.CheckFillInText(new Control[] { txbUserName }))
            {
                DialogResult result = MessageBox.Show($"Bạn có chắc chắn muốn xóa nhân viên {txbName.Text} với tên đăng nhập {txbUserName.Text} không?",
                    "Thông báo", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2);
                if (result == DialogResult.OK)
                {
                    try
                    {
                        bool check = AccountDAO.Instance.DeleteAccount(txbUserName.Text);
                        if (check)
                        {
                        ...
                        {
                            MessageBox.Show("Không thể xóa (Tên đăng nhập không tồn tại)", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                    catch(Exception ex)
                    {
